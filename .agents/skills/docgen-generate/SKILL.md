...
```
- This will return a JSON with a `fields` array containing all detected variables (even if they are just plain text, the LLM will identify them).

### 2. Generate PDF from DOCX
Use this to fill the DOCX template with data and get a PDF back.
```bash
curl -X POST   -F "file=@template.docx"   -F "payload={\"client_name\":\"Budi\", \"total\":\"Rp 500.000\"}"   http://localhost:3000/api/v1/docx/generate --output result.pdf
```
- `file`: The DOCX template file
- `payload`: A JSON string containing the key-value pairs to inject
- `options`: (Optional) JSON string for watermark settings e.g. `{"watermark":{"enabled":true,"text":"DRAFT"}}`
